...
バッチファイル内部はWindows 10のcmd.exeでUTF-8由来の文字化けや構文エラーが起きないように、コメント、表示メッセージ、ラベル名を半角英数字だけで記述しています。成功時・失敗時のどちらも `pause` せずに終了します。Dropbox内など、パスに日本語や空白が含まれる場所でも動作するように、各パスは引用符付きで処理しています。
